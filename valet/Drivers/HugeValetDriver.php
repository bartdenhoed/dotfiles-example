...
class HugeValetDriver extends ValetDriver
{
    /**
    ...
    {
        // HugeFramework required both files
        if (
            file_exists($sitePath . '/application/config/config.php') && 
    ...
    public function frontControllerPath($sitePath, $siteName, $uri)
    {
        ...
        foreach ($fixedCandidatesAndDocroots as $candidate => $docroot) {
            if ($this->isActualFile($candidate)) {
                // Replace get URL variable like .htaccess does
                if (!isset($_GET['url']) && !empty($uri) && $uri !== '/') {
                  $_GET['url'] = $uri;
                }
